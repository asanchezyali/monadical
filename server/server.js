...
io.on('connection', socket =>{
    socket.on('newGame', () => {
        try {
            ...
            console.log('On the client submit event (on start page) to create a new room')
        }
    })
    ...
    socket.on('joining', ({room}) => {
        try {
            ...
            console.log('On the client submit event (on start page) to join a room')
        }
    })
    ...
    socket.on('newRoomJoin', ({room, name})=>{
        ...
        try {
            ...
            console.log('redirect them back to the start page')
        }
    })
    ...
    socket.on('move', ({room, piece, index}) => {
        try {
            ...
            console.log('Listener event for each move and emit different events depending on the state of the game')
        }
    })
    ...
    socket.on('playAgainRequest', (room) => {
        try {
            ...
            console.log('Reassign new piece so a player cant always go first')
        }
    })
    ...
    socket.on('disconnecting', ()=> {
        try {
            ...
            console.log('Get all the rooms that the socket is currently subscribed to')
        }
    })
